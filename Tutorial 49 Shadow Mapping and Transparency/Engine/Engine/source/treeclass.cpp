...
bool TreeClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* trunkModelFilename, char* trunkTextureFilename, char* leafModelFilename, char* leafTextureFilename, float scale)
{
	bool result;
	...
	result = LoadTextures(device, deviceContext ,trunkTextureFilename, leafTextureFilename);
	if(!result)
	{
		return false;
	}
...
bool TreeClass::LoadTextures(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* trunkFilename, char* leafFilename)
{
	bool result;
	...
	result = m_TrunkTexture->Initialize(device, deviceContext, trunkFilename);
	if(!result)
	{
		return false;
	}
	...
	result = m_LeafTexture->Initialize(device, deviceContext, leafFilename);
	if(!result)
	{
		return false;
	}
